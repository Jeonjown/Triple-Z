...
const ScheduleHistory = (): JSX.Element => {
  ...
  // Retrieve event settings for the corkage fee (though we'll primarily use saved values).
  ...
  if (!user)
    return (
      <p className="p-4 text-center">
        Please log in to view your schedule history.
      </p>
    );
  if (isLoading)
    return <p className="p-4 text-center">Loading reservation history...</p>;
  if (error)
    return (
      ...
      <Table className="min-w-full">
        <TableHeader>
          <TableRow className="border border-gray-300 bg-gray-100">
            ...
            {/* Changed from Balance Due to Payment Link */}
            <TableHead className="px-3 py-2 text-left text-xs font-bold text-gray-700 sm:px-4 sm:text-sm">
              ID
            </TableHead>
            <TableHead className="px-3 py-2 text-left text-xs font-bold text-gray-700 sm:px-4 sm:text-sm">
            ...
            sortedReservations.map((reservation: Reservation) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Payment Link */}{" "}
                  {/* Replaced Balance Due with Payment Link */}
                  <TableCell className="px-3 py-2 text-xs sm:px-4 sm:text-sm">
